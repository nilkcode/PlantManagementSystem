...
// =====================
const fetchMachineList = async () => {debugger
    try {
...
const renderMachineTable = () => {debugger
    ...
    let updatedMachineData = setDataAsPerPagination()


    tableBody.innerHTML = '';
    
    updatedMachineData.forEach(machine => {
        const rowHTML = `
...
const setDataAsPerPagination = () => {
    debugger
    totalPages = Math.ceil(machineData.length / rowPerPage)
    start = 0    //  (4 - 1) * 5 = 3 * 15  start = 15   ()
    let end = start + rowPerPage;           //  15 + 5 = 20  end  = 20
    let paginatedData = machineData.slice(start, end);
    document.querySelector("#pagination-count").innerHTML = ` Showing <span class="font-semibold text-heading">${start + 1}</span> of <span class="font-semibold text-heading">${end}</span>`
    document.querySelector("#paginationNumber").innerHTML = ` <li>
                                    <a href="#" id="prev"
                                 ...
                                 ${Array.from({ length: totalPages }, (_, i) => `<li>
                                    <a href="#"
                                        class="flex items-center justify-center text-body bg-neutral-secondary-medium box-border border border-default-medium hover:bg-neutral-tertiary-medium hover:text-heading font-medium text-sm w-9 h-9 focus:outline-none">${i + 1}</a>
                                </li>`)}
...
const nextPage = () => {
    ...
    // paginatedData = machineData.slice(start, end);
     fetchMachineList(10)
}
